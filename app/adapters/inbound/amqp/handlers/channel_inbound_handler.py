...
class ChannelInboundHandler(MessageHandler):
    ...
    Resolves (or creates) tenant, contact and session, then enqueues into the debounce buffer.
    """
    ...
    async def handle(
        ...
        # phone_number_id (Meta) or any platform recipient id becomes the tenant slug
        tenant_slug = inbound.tenant_id or DEFAULT_TENANT_SLUG
